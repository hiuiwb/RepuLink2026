...
def save_scores(users, scores, output_file):
    ...
    df_out = pd.DataFrame({
        "dst": users,
        "eigentrust_score": scores
    })
    df_out.to_csv(output_file, index=False)
    print(f"EigenTrust scores saved to {output_file}.")

def main():
    # 参数设定：文件路径和参数
    input_file = "/ECShome/ww3y23/Github/Repulink/datasets/bitcoin_alpha.csv"  # 输入数据文件
    output_file = "eigentrust_scores.csv"   # 输出文件
